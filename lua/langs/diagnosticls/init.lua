...
function M.init_diagnosticls()
	local function start_diagnosticls(server)
		server:setup({
			...
			autostart = true,
			filetypes = M.conf.lsp_ft,
			on_attach = function(client, bufnr)
				if client.resolved_capabilities.document_formatting then
					global.fn.autocmd("LspAutocommands", {
						[[BufWritePre <buffer> lua vim.lsp.buf.formatting_seq_sync()]]
					}, true, true)
				end
			end,
			init_options = {
				filetypes = M.conf.linters_ft,
				linters = M.conf.linters,
				formatFiletypes = M.conf.format_ft,
				formatters = M.conf.formatters,
			},
	...
	langs_setup.setup_lsp("diagnosticls", start_diagnosticls)
end
