...
function M.generate_ftplugin(server_name, dir)
  ...
  for _, filetype in ipairs(filetypes) do
    local filename = Path(dir, filetype .. ".lua")
    local setup_cmd = ([[require("lsp.manager").setup(%q)]]):format(server_name)
    -- print("using setup_cmd: " .. setup_cmd)
    -- overwrite the file completely
    WriteFile(filename, setup_cmd .. "\n", "a")
  end
end
...
function M.generate_templates(servers_names)
  ...
  vim.notify("Templates installation in progress")
  ...
  vim.notify("Templates installation is complete")
end
