-- https://github.com/LunarVim/LunarVim/blob/rolling/lua/lvim/lsp/peek.lua
local M = {
  floating_buf = nil,
  floating_win = nil,
  prev_result = nil,
...
local function create_floating_file(location, opts)
  ...
  opts = vim.lsp.util.make_floating_popup_options(if_nil(width, 50), if_nil(height, 10), opts)
  ...
  vim.api.nvim_command(
    ("autocmd %s <buffer> ++once lua pcall(vim.api.nvim_win_close, %d, true)"):format(unpack(close_events), winnr)
  )
  return bufnr, winnr
end
