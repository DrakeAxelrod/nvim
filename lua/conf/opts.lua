conf.opts = {}
--[[----------------------------------------------------------------------]]
---Things you need to set quickly
---@type function
--[[----------------------------------------------------------------------]]
conf.opts.quick = function()
  vim.api.nvim_set_var("did_load_filetypes", true)
  vim.api.nvim_set_var("cursorhold_updatetime", 100)
end
--[[----------------------------------------------------------------------]]
---built-in plugins we don't wanna load
---@type function
--[[----------------------------------------------------------------------]]
conf.opts.disables = function()
  -- zip
  vim.g.loaded_gzip = 1
  vim.g.loaded_zip = 1
...
--[[----------------------------------------------------------------------]]
---UI Options that have to do with the user interface
---@type function
--[[----------------------------------------------------------------------]]
conf.opts.ui = function()
...
--[[----------------------------------------------------------------------]]
---Behavior Options that have to do with behavior
---@type function
--[[----------------------------------------------------------------------]]
conf.opts.behavior = function()
...
--[[----------------------------------------------------------------------]]
---File Options that have to do with files
---@type function
--[[----------------------------------------------------------------------]]
conf.opts.files = function()
  local cache_path = vim.fn.stdpath("cache") .. "/xyz"
  local data_path = {
    api.fs.join(cache_path .. "/backup"),
    api.fs.join(cache_path .. "/session"),
    api.fs.join(cache_path .. "/swap"),
    api.fs.join(cache_path .. "/tags"),
    api.fs.join(cache_path .. "/undo"),
  }
  if not api.fs.exists(cache_path) then
    os.execute("mkdir -p " .. cache_path)
  end
  for _, v in pairs(data_path) do
    if api.fs.isdir(v) then
      os.execute("mkdir -p " .. v)
    end
  end
  vim.opt.directory = api.fs.join(cache_path .. "/swag")
  vim.opt.undodir = api.fs.join(cache_path .. "/undo")
  vim.opt.backupdir = api.fs.join(cache_path .. "/backup")
  vim.opt.viewdir = api.fs.join(cache_path .. "/view")
  vim.opt.spellfile = api.fs.join(cache_path .. "/spell/en.uft-8.add")
  vim.opt.shadafile = api.fs.join(cache_path .. "/shada")
...
--[[----------------------------------------------------------------------]]
---Misc Options that have to do misc things
---@type function
--[[----------------------------------------------------------------------]]
conf.opts.misc = function()
...
--[[----------------------------------------------------------------------]]
---Lists Options that have to do lists
---@type function
--[[----------------------------------------------------------------------]]
conf.opts.lists = function()
  vim.opt.list = true
  vim.opt.listchars = {
    -- eol = "↲",
    tab = "⎵/",
