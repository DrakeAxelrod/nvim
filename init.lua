...
setup({
  ...
  mappings = {
    ...
    packages = {
      ["smart-splits"] = function(mod, desc)
        return {
          --> resize buffers <--
          { { "n" }, "<s-h>", mod.resize_left, desc "Resize buffer left" },
          { { "n" }, "<s-j>", mod.resize_down, desc "Resize buffer down" },
          { { "n" }, "<s-k>", mod.resize_up, desc "Resize buffer up" },
          { { "n" }, "<s-l>", mod.resize_right, desc "Resize buffer right" },
          --> swap buffers <--
          { { "n" }, "<c-h>", mod.move_cursor_left, desc "Cursor to left buffer" },
          { { "n" }, "<c-j>", mod.move_cursor_down, desc "Cursor to bottom buffer" },
          { { "n" }, "<c-k>", mod.move_cursor_up, desc "Cursor to top buffer" },
          { { "n" }, "<c-l>", mod.move_cursor_right, desc "Cursor to right buffer" },
          { { "n" }, "<leader>Br", mod.start_resize_mode, desc "Resize Mode" },
        }
      end,
      --> Package Manager <--
      ["dep"] = function(_, desc)
      ...
      ["neogit"] = function(_, desc)
        return {
          { { "n" }, "<leader>gg", ":Neogit<cr>", desc "Open Neogit" },
        }
      end,
